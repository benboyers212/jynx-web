...
// GET /api/workouts — list all WorkoutHubs for the current user
export async function GET() {
  const dbUserId = await getDbUserId();
  if (!dbUserId) return NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 });

  const hubs = await prisma.workoutHub.findMany({
...
// POST /api/workouts — create a WorkoutHub
export async function POST(req: NextRequest) {
  ...
  const hub = await prisma.workoutHub.create({
    data: {
      userId: dbUserId,
      name,
