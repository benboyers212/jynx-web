...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Tune values EXACTLY here (these are the “numbers” you asked for)
  const headerT = useMemo(() => clamp(scrollY / 140, 0, 1), [scrollY]); // 0 → 1 by 140px
  const bgAlpha = lerp(0.84, 0.62, headerT); // header background fades as you scroll
  const borderAlpha = lerp(0.08, 0.045, headerT); // border gets quieter
  const washOpacity = lerp(0.14, 0.05, headerT); // olive wash fades
  const topHighlightAlpha = lerp(0.012, 0.006, headerT); // subtle highlight fades
  const navIdleAlpha = lerp(1.0, 0.88, headerT); // tabs/icons “relax” on scroll
  const navActiveAlpha = lerp(1.0, 0.92, headerT); // active stays prominent, slightly relaxes
  const blurPx = lerp(16, 10, headerT); // backdrop blur eases down a bit
    ...
    <html lang="en" className={dmSans.variable}>
      <body className="font-sans bg-neutral-950 text-neutral-100">
        ...
        <div className="sticky top-0 z-40">
          ...
          >
            {/* subtle olive wash so it matches the schedule vibe */}
            ...
            <div className="relative mx-auto max-w-7xl px-6">
              {/* slightly shorter than before (still comfy) */}
              <div className="h-16 flex items-center">
                {/* Left: brand (anchored left) */}
                <div className="flex items-center gap-3 min-w-[220px]">
                  ...
                  <div className="min-w-0">
                    {/* Make Jynx WHITE + remove tagline */}
                ...
                <div className="min-w-[220px] ml-auto flex items-center justify-end gap-2">
                  ...
                  <button
                    onClick={() => alert("Profile (UI shell)")}
                    className="h-10 w-10 rounded-full border border-white/12 bg-white/6 hover:bg-white/10 transition flex items-center justify-center"
                    style={{
                      opacity: navIdleAlpha,
                      boxShadow: "0 0 0 1px rgba(85,107,47,0.18)",
              ...
              >
                <div className="h-12 px-4 flex items-center border-b border-white/10 shrink-0">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    {/* NEW: expand button next to close */}
                    <button
                ...
                {/* CRITICAL: force FilesPage root <main> to size to this modal, not the viewport */}
...
/** Better “files” icon (reads like a folder) */
